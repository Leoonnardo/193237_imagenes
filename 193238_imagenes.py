...
if __name__ == "__main__":
   direcciones()
   print(f"Descarga multi hilos: {timeit.Timer(multi_hilos).timeit(number = 1)}")
   print(f"Descarga multi procesos: {timeit.Timer(multi_procesos).timeit(number = 1)}")
   print(f"Descarga normal estructurado: {timeit.Timer(normal_estructurado).timeit(number = 1)}")